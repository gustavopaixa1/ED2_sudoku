...
static void swap(pairEDM** a, pairEDM **b) {
    pairEDM* temp = *a;
    *a = *b;
    *b = temp;
}
...
static int partition(arrayEDM* interpol, int low, int high) {
...
// Função do Quicksort
static void quicksort(arrayEDM* interpol, int low, int high) {
    if (low < high) {
...
static void insert(char* start_game, char* solution, arrayEDM** interpol) {
    ...
    if ((*interpol)->tamanho_do_array == (*interpol)->totalElementos) {
        ...
        temp = realloc((*interpol)->hash_map, novoTamanho*sizeof(pairEDM*));
    ...
    (*interpol)->hash_map[index] = malloc(sizeof(pairEDM));
...
// Funções public--------------------------------------------------------
arrayEDM* buildEDM(int _size) {
    
    arrayEDM *my_array_inter = malloc(sizeof(arrayEDM));
    ...
    my_array_inter->hash_map = malloc(sizeof(pairEDM*));

    // Verifica se a alocação do hash_map foi bem-sucedida
    if (my_array_inter->hash_map == NULL) {
        free(my_array_inter);  // Libera a memória de arrayEDM antes de retornar
        return NULL;
    }
...
static void destroyEDMHashMap(pairEDM** hash_map, int total) {
...
void destroyEDM(arrayEDM* interpol, int inst_size) {
    // TODO Libera espaço de memória associado

    destroyEDMHashMap(interpol->hash_map, interpol->totalElementos);
    free(interpol);
}

void leituraEDM(char* instancia, arrayEDM** interpol, char*** input, char*** output, int *inst_size) {
...
void execEDM(char* instancia, arrayEDM* arr, char*** respostas, int *inst_size) {
    ...
    if (fscanf(file, "%d\n", inst_size) != 1) {
        perror("Erro ao ler o número de strings");
        fclose(file);
        return;
    }
    ...
    *respostas = malloc((*inst_size) * sizeof(char*));
    ...
    fclose(file);
    printf("Leitura concluída: %d strings carregadas.\n", *inst_size);
}
// ----------------------------------------------------------------------